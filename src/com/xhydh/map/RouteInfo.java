...
public class RouteInfo extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (Daohang.distance<1000) {
			textView.setText("总计："+Daohang.distance+"m");
		}else {
			textView.setText("总计："+(((float)(Daohang.distance))/1000)+"km");
		}
		//listView.setAdapter(new ArrayAdapter<String>(getApplicationContext(), android.R.layout.simple_list_item_1, R.id.route_info,Daohang.route_line));
			...
			public View getView(int position, View view, ViewGroup arg2) {
				...
				String string = Daohang.route_line.get(position);
			...
			public Object getItem(int position) {
				return Daohang.route_line.get(position);
			}

			@Override
			public int getCount() {
				return Daohang.route_line.size();
			}
		});
		}
